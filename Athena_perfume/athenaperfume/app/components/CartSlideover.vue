<template>
  <USlideover v-model:open="isOpen" side="left" title="سبد خرید">
    <template #body>
      <!-- سبد خالی -->
      <div v-if="cart.items.length === 0" class="flex flex-col items-center justify-center h-full text-center px-6 py-12">
        <UIcon name="i-lucide-shopping-cart" class="w-20 h-20 text-gray-400 mb-6" />
        <p class="text-xl font-bold mb-3">سبد خرید خالیه!</p>
        <p class="text-gray-500 mb-8">هنوز چیزی انتخاب نکردی</p>
        <UButton to="/products" color="primary" size="lg" @click="isOpen = false">
          برو به فروشگاه
        </UButton>
      </div>
      ...
      <div v-else class="space-y-4 pb-20">
        ...
        >
          <img v-if="item.image" :src="item.image" class="w-20 h-20 rounded-lg object-cover" />
          <div class="flex-1">
          ...
          <div class="flex flex-col justify-between items-end gap-3">
            <div class="flex items-center gap-2 bg-white dark:bg-gray-900 rounded-lg">
              <UButton icon="i-lucide-minus" size="xs" @click="decrement(item)" />
              <span class="w-10 text-center font-bold">{{ item.qty }}</span>
              <UButton icon="i-lucide-plus" size="xs" @click="cart.addToCart(item, 1)" />
            </div>
            <UButton
    ...
    <template #footer v-if="cart.items.length">
      <div class="space-y-4 border-t pt-6">
        <div class="flex justify-between text-lg font-bold">
          <span>جمع کل:</span>
          <span class="text-primary">{{ formatPrice(cart.totalPrice) }} تومان</span>
        </div>
